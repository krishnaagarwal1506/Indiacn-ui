...
import { Children, ComponentProps, ReactNode, createContext, useContext, useMemo } from 'react';
...
function AvatarGroup({
  ...
  const groupContext = useMemo(() => ({ size, shape, inGroup: true }), [size, shape]);

  return (
    <GROUP_CONTEXT.Provider value={groupContext}>
      <div className={cn('flex items-center', className)}>
        {visible.map((child, i) => (
          <div key={i} className={cn('flex', i > 0 && GROUP_OVERLAP[size])}>
            {child}
          </div>
        ))}
        {overflow > 0 && (
